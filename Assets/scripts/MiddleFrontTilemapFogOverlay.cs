...
/// <summary>
/// Always overlays fog 0.1 units in front of the middle front tilemap,
/// no matter the tilemap's Z position or cell.z values.
/// Fog overlays all active tiles in the middle front tilemap.
/// </summary>
public class MiddleFrontTilemapFogOverlay : MonoBehaviour
{
    [Header("References")]
    ...
    void Update()
    {
        if (middleFrontTilemap == null || fogTilemap == null || fogTile == null ||
            worldSpawner == null || tileHiddenSet == null || mainCamera == null)
            return;

        Vector3 middleFrontPos = middleFrontTilemap.transform.position;
        Vector3 fogPos = fogTilemap.transform.position;
        float desiredZ = middleFrontPos.z + 0.1f;
        ...
        Vector3 srcOrigin = middleFrontTilemap.layoutGrid.CellToWorld(Vector3Int.zero) + middleFrontTilemap.transform.position;
        ...
        HashSet<Vector3Int> activeTiles = worldSpawner.GetActiveTilesForTilemap(middleFrontTilemap);
        HashSet<Vector3Int> toHide = tileHiddenSet.GetTilesToHide(middleFrontTilemap.transform.position);
        ...
        foreach (var tile in activeTiles)
        {
            if (tile.x < minX || tile.x > maxX || tile.y < minY || tile.y > maxY)
                continue;
            if (middleFrontTilemap.GetTile(tile) == null)
                continue;

            Vector3 worldPos = middleFrontTilemap.CellToWorld(tile) + worldOffset;
